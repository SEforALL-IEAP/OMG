...
def centroids_to_extract_solar_data(clusters_geometry):
    """
    Computes centroids for a given GeoDataFrame of geometries and returns a GeoDataFrame of points representing these centroids. If the number of geometries is
    greater than 50, KMeans clustering is applied to reduce the number of centroids to 50.
    
    Parameters:
    clusters_geometry (GeoDataFrame): A GeoDataFrame containing clusters of geometries.
    
    Returns:
    GeoDataFrame: A GeoDataFrame containing the computed centroids or clustered centroids.
    """
    
    # Create a copy of the input GeoDataFrame to avoid modifying the original
    clusters_geometry_centroids = clusters_geometry.copy()
    
    # Calculate the centroids of the geometries and store them in a new column
    clusters_geometry_centroids["geometry"] = clusters_geometry_centroids.centroid

    # If the number of clusters is 50 or less, use their centroids directly
    if len(clusters_geometry) <= 50:
        # Create a GeoDataFrame with IDs and centroid geometries
        gpd_points = gpd.GeoDataFrame(
            {'id': range(len(clusters_geometry_centroids)), 
             'geometry': clusters_geometry_centroids['geometry']},
            crs=clusters_geometry.crs
        )
    else:
        # If more than 50 clusters, compute centroids for KMeans clustering
        clusters_geometry["coord_x"], clusters_geometry["coord_y"] = clusters_geometry_centroids.centroid.x, clusters_geometry_centroids.centroid.y

        # Create a NumPy array of the centroid coordinates
        points_np = np.array([clusters_geometry["coord_x"], clusters_geometry["coord_y"]]).T
        
        # Perform KMeans clustering to reduce the number of points to 50
        kmeans = KMeans(n_clusters=50)
        kmeans.fit(points_np)
        new_points = kmeans.cluster_centers_
        
        # Create a new GeoDataFrame with IDs and the clustered points' geometries
        gpd_points = gpd.GeoDataFrame(
            {'id': range(len(new_points)), 
             'geometry': [Point(gm) for gm in new_points]},
            crs=clusters_geometry.crs
        )
        
    # Return the GeoDataFrame containing the centroids or clustered centroids
    return gpd_points.to_crs(4326)
...
def getting_pv_data_bulk(clusters, workspace_pv, token):
    """
    ...
    points_for_solar_data = centroids_to_extract_solar_data(clusters)
